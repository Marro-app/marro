...
- **Hit targets ≥ 44×44 pt** (Apple's minimum — stricter than WCAG's 24px; use Apple's). ✓ Resolved (2026-07-20): `.xbtn`/`.infotip-btn` ✕ and "i" controls already reach 44px via `::after` hit-slop despite their small 16–28px visuals; the two that had slipped through — the reusable `InfoNote` dismiss ✕ and BudgetTab's "Add category" — were moved onto the hit-slop pattern (`.hit-slop`, see Buttons below).
...
**Hit targets** — every button needs a real 44×44pt clickable area (Apple HIG; stricter than WCAG 2.2's 24×24px floor, which most serious products exceed anyway). Where the visible control must stay small for density (`.xbtn` icon-only ✕ buttons render at 26-30px to stay visually quiet in tight rows), the fix is to expand the *invisible hit area* via a pseudo-element (`.xbtn::after { inset:-8px }`) rather than growing the icon itself — same visual footprint, real 44px target underneath. For small standalone text/pill actions that aren't `.xbtn` (e.g. "+ Add category"), add the shared **`.hit-slop`** utility (`index.html`: `position:relative` + `::after{inset:-11px}`, lifts a ~22px control to 44px) — same technique, reusable. **Never** put `.hit-slop` on an inline text link inside a sentence (e.g. "Manage" within a subscription-count line): its vertical slop would overlap neighboring rows, and inline links are exempt from the target-size rule anyway.
