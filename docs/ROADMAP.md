...
- ✓ **Automatic click-tracking + Admin Usage dashboard** (2026-07-29, branch `ethan/analytics`) — a single global click listener (`src/lib/analytics.js`) records a `ui_click` event for every button/link click app-wide with zero per-feature setup, plus a founder-facing **Usage** section in the Admin tab (most/least-clicked elements by tab, a daily activity trend, active-clicker count), all built on three admin-gated RPCs in `supabase/analytics.sql`. Details in `PRODUCT_DECISIONS.md` 2026-07-29.
