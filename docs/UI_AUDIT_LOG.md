...
- **Bug/Idea are forms, not chat** (per founder feedback): picking Bug or Idea swaps the chat composer for a short labelled form (each `<textarea>` has a real `<label htmlFor>`, required field enforced, `role="alert"` validation, ≥44px "Submit" primary). Submit composes the fields into the first message body and shows a confirmation ("Thanks — we got your bug report", `Done` closes). Question stays a chat. Form/confirmation verified keyboard-reachable in both themes.
- **Resume-link lifecycle** (per founder feedback — don't let "Back to your conversation" linger forever, and don't offer it for one-and-done bug/idea submissions): the panel now auto-opens a thread only when a reply is *waiting* (unread, any type); otherwise the picker. The "Back to your conversation" link shows only for a **Question** that is still open and was active within the last 7 days — so it drops off once the question is resolved or goes quiet, and bugs/ideas never show it. Verified: seed question shows the link; after submitting a bug, reopening shows the picker with no link.
- **Single-active-chat model + end/reopen** (per founder decision): a user may have only one open **Question** chat at a time (bugs/ideas remain unlimited one-off submissions). The picker's Question card becomes **"Continue your chat"** (with unread badge) when one's active; the composer to start a new one only shows when none is. In the chat: a **back arrow** (`aria-label="Back to menu"`) returns to the picker so Bug/Idea stay reachable, and an **"End chat"** action opens an inline **confirmation** (Cancel outlined left / "End chat" danger fill right — rule 9) before archiving. Ending shows a "Chat ended" confirmation (the future CSAT slot) → picker, where **"Reopen your recent chat"** appears for the archived thread (7-day window). All new controls are real `<button>`s, ≥40–44px, keyboard-reachable with the app `:focus-visible` ring; the End-chat danger button uses `C.bg`-on-`C.danger` (~6–8:1, both themes, same as the badge fix). Verified the whole loop in `?mock=1`: continue → end → confirm → ended → reopen, plus Bug submittable while a chat is active.
- **Hub-and-spoke restructure** (per founder feedback — the 3-card picker felt like a detour, and the bug/idea screens shouldn't show the other options): the panel now opens to a **hub** (home) with three full-width rows — *Ask a question / Report a bug / Share an idea*. Each row navigates to its **own screen** (chat composer / bug form / idea form), which shows only that flow plus a **back arrow** to the hub. Motifs live on the destination screens; the hub is neutral. Verified in `?mock=1`: hub → Report a bug (form only + back) → back → hub (all three). All rows/back are real `<button>`s, ≥52–58px, keyboard-reachable with the app `:focus-visible` ring.
- **Separate Continue card + start-new guard** (per founder feedback): an open chat gets its own prominent **"Continue your chat"** card above the menu (with unread badge). "Ask a question" no longer silently continues — with a chat open it routes to an **askChoice** screen ("You have an open chat" → *Continue that chat* / *Close it & start a new one*); closing archives the current chat and opens a fresh composer. Verified the whole loop in `?mock=1`.
- **Recent chats list** (per founder feedback — show *all* past chats, not just the most recent): the hub now lists **every** chat ended within the last 7 days (`findReopenableChats`, newest-first), each a labelled row (subject + "Ended 1d ago · tap to reopen", `aria-label` carries both) that reopens on tap. Shown only when no chat is active (single-active rule); reappears once the current chat is ended. Verified with a 3-chat seed in `?mock=1`: list renders with correct relative times, tapping reopens the thread.
- **Grant hardening (security, rule 4)**: found the support user-RPCs were executable by `anon` (Supabase default privileges grant EXECUTE to anon on new functions, and `revoke … from public` doesn't remove that explicit grant). The RPCs self-guard (`auth.uid()` null → no-op), so no data was exposed, but they now `revoke … from public, anon` explicitly — verified on prod that `anon` execute is `false` and `authenticated` is `true` for all five support RPCs (this also hardened the Slice-1 RPCs).
...
- **Root-caused the flaky "remove category" click** (`src/tabs/BudgetTab.jsx`): the whole row was `draggable`, so trackpads especially would claim a click+tiny-move as `dragstart` and silently eat the ✕ click. Moved `draggable`/`onDragStart` onto the ⠿ handle only (now a real `<button>`); the row keeps `onDragOver`/`onDrop` so mouse drag-drop is unaffected. Verified: clicked "Remove" on 3 different categories back-to-back, confirm dialog opened every time (previously intermittent).
- **Keyboard reorder path (WCAG 2.1.1)**: the ⠿ handle is now a focusable `<button aria-label="Reorder <category>: use arrow keys">` with a real `:focus-visible` ring (inherited from the app-wide `button:focus-visible` rule — no new CSS needed); ArrowUp/ArrowDown call the existing `reorderCats()` against the same filtered/ordered list the row map uses, so keyboard and mouse-drag stay in sync. No aria-live announcement added — the app has no existing live region for this and the instruction was to skip building new infra; the reordered DOM position + retained focus (same React `key`) is itself the feedback. Verified: focused Transportation's handle, ArrowDown moved it below Personal, ArrowUp moved it back, focus never left the handle.
- **Quick Add focus loss, and the same bug in every settings-menu-launched modal**: `Modal` (`src/components/primitives.jsx`) captured "what had focus before I opened" in its **mount `useEffect`**, which runs *after* React applies a child's `autoFocus` — so it captured the just-autofocused field, not the real trigger, and on close focus fell to `<body>`. Separately, the same effect **unconditionally** force-focused `focusables()[0]` (almost always the ✕ close button), stomping any `autoFocus` field a split second after the browser had already honored it. Fixed both: `prevFocus` is now captured via a lazy `useState(() => document.activeElement)` initializer (runs during render, before commit/autoFocus), and the force-focus only fires if nothing inside the panel already has focus. Root-caused a *second*, unrelated bug this exposed: every settings-menu row (Categories, avatar, school, program) closes the dropdown **and** opens its modal in the same click handler/commit, so the menu row Modal tries to refocus on close has already been unmounted — `.focus()` on a detached node is a silent no-op. Fixed by giving the Settings gear button a ref and having those four modals explicitly refocus it on close instead of relying on generic restore (`src/App.jsx`). Verified: Quick Add opens with the category `<select>` focused, Esc/✕ returns focus to the Quick Add button; Categories modal opens and closes back to the Settings gear button (previously dropped to `<body>`, confirmed via `document.activeElement` before/after the fix).
- **Money input validation**: negative entries could reach state and corrupt totals (e.g. a negative Aid "Housing (monthly)" flipping the Monthly Plan sign). Added `sanitizeMoneyInput(raw, max)` (`src/lib/format.js`) — strips a leading minus at the input layer (can't type a negative dollar figure) without reformatting mid-decimal-entry (`"12."` passes through), and optionally clamps down to `max` once the value parses to a real number. Applied to: Aid's 4 money fields + housing, Budget's category-amount inputs, the Loans balance check-in (spendable/savings) and per-loan amount fields (same bug class, found while there), and Quick Add's amount field, which also gets a hard cap at $999,999 with a small "Capped at $999,999." note beneath the field (was silently accepting $999,999,999 and breaking the chart axis). 5 new Vitest cases for `sanitizeMoneyInput`. Verified: typed `-9999` into Aid Housing → field shows `9999`; typed `999999999` into Quick Add Amount → clamped to `999999` with the note visible.
- Tests: 163/163 passing (158 baseline + 5 new) · build clean · lint 0 errors (109 pre-existing warnings, none new). Both themes screenshotted (Budget category list + reorder handle focus ring, Aid Housing field, Quick Add cap note).
...
## 2026-07-18 — Copy-clarity pass (branch `mo/copy-clarity`, amending marro-copy-audit.md findings)

Verified via a temporary isolated worktree (origin/mo/dev-test-harness's mock session, applied as a diff against this branch's changes; worktree deleted before commit, per rule — `mo/dev-test-harness` (PR #34) is not yet merged into main). Walked Budget, Aid & Detail, Loans in both themes, desktop and 375px mobile.

- **Killed all 5 "disbursed" instances** (the audit's #1 jargon-table violation): Budget cash-flow row → "Total aid sent to you"; Aid year-card summary box → "Sent to you/yr"; Aid overview table header → "Sent to you/yr"; Categories → Key notes → "…is sent to you —"; the Aid info-banner instance was subsumed by the equation redesign below. Internal field names (`annDisburse`, loan `status:'disbursed'`, etc.) are untouched — copy only, per scope.
- **Reframed the Aid tab's "4-year net: −$102,040"**: number and label kept as-is; added a calm reassurance line beneath (shown only when the total is negative) — "Most med students borrow to bridge this — that's what the loans are for. Your aid office can help you plan it." (`src/tabs/AidTab.jsx`). Verified numerically that the existing color pairing here is already the amber/neg token, not the danger/red token (theme.js documents `neg` as "Negative DATA… amber", distinct from `danger`/"warm clay" used only for destructive actions/errors) — no color change was needed, just the added text so the number's meaning isn't color-only.
- **Redesigned the Aid info-banner** as a real vertical breakdown using the active year's live numbers: Total aid / − Tuition & fees / − Health insurance (only shown if >0) / a highlighted "= You get $N for living costs" row using the existing tealLight/teal money-positive idiom (same tokens the year-card summary box already uses). Verified at 375px — wraps cleanly, no overflow, ≥4.5:1 contrast in both themes (spot-checked numerically: teal-on-tealLight ~5.5:1 dark / ~4.9:1 light).
- **De-cluttered the 4 Aid year cards**: each now collapses to a summary row (year label, Total aid, Sent to you, the surplus Pill) via a real `<button aria-expanded aria-controls>` — native focus + Enter/Space activation, no custom key handling needed (same idiom as LoansTab's existing "More options" disclosure). Defaults: the year whose date range contains today expands automatically; if none does, falls back to the active tab year, then the first year. Verified: expand/collapse by mouse and keyboard on both Year 1 (auto-expanded, in-range) and Year 2 (collapsed by default) in the mock; date-picker inputs live inside the expanded region only (kept out of the toggle `<button>` — interactive controls can't nest inside a button). The 4-year overview table itself is untouched except its "Disbursed/yr" → "Sent to you/yr" header (same rename as item 1).
- **InfoTip pass** (existing `InfoTip` component, `src/components/primitives.jsx`): added to "Sent to you/yr" (Aid year-card box), "Planned surplus" (both the Budget cash-flow row and the dedicated Planned-surplus card — the card's old `sub="if you stay on budget"` was removed since the new tip fully covers that meaning plus the bank-balance distinction the sub didn't; the cash-flow row's sub was kept since it also carries "through {month}" scope info the tip doesn't), and the Loans tab's federal-rate line. Verified hover + click-toggle + Esc-to-close on the Budget instances; the component's keyboard behavior is unchanged code, reused as-is elsewhere.
- **Small clarity touches**: the Aid year-card Pill (previously a bare "$X/mo") now appends "short"/"left over"; the Runway tile's `counting_down` sub-line now leads with "how long your money lasts —" before the date; Budget's 4 "Health checks" rows now fold behind a single expandable "Health checks" toggle (same disclosure pattern as above), collapsed by default.
- **Emoji-as-UI-chrome removed** (founder scope addition, CLAUDE.md rule 8 — inconsistent cross-platform rendering, can't follow theme tokens): ⏳ on the Loans 120-day return-window card (icon dropped, card already has a themed left-border accent as its visual anchor), 📬 on the Refund Playbook heading, 🎓 on the Runway tile's "all done" state, 🎉 on the invite-gate welcome banner. Left untouched: user-selectable avatar/category emoji, and the plain ✓/✕/⚠ dingbat glyphs used elsewhere (monochrome, follow `currentColor`/theme tokens, not the colorful-emoji rendering problem this addition targeted).
- **Flagged, not fixed (out of scope, spawned as a separate task)**: a numeric contrast check found the light-theme `neg`-on-`negLight` pairing (used by warn-state Pills and the "4-year net" box's own background) computes to ~3.65:1 against its actual blended background — below the 4.5:1 AA minimum. This is a pre-existing token pairing (unchanged by this PR) already used elsewhere in the app before this branch; fixing it means retuning a shared design token across every screen that uses it, which is outside this PR's "exactly this" scope. Filed for follow-up rather than silently patched.
- **Verified untouched (GOOD list, per audit)**: Loans empty state, HPSL banner, "arrives in two parts" helper, balance check-in's "No bank login" line, the graduation debt card's "what you'll owe… interest included."
- Tests: 152/152 passing · build clean · lint 0 errors (109 pre-existing warnings, none new). Both themes + 375px spot-checked for every changed surface; no console errors observed.
...
## 2026-08-11 — Screenshot Studio object-model rearchitecture (branch `review-slice-10`, PR #82)

Added two tools (Select, Move) + a Delete affordance to `ScreenshotStudio.jsx`'s annotation toolbar, plus two optional text inputs (Name/Summary) before "Attach this image." Same icon/toolbar language as the existing 5 tools (20×20 line icons, `aria-pressed` on every tool button, `role="group"` on the toolbar and the color swatches) — no new patterns introduced.

- **New icons** (`src/components/icons.jsx`): `toolSelect` (filled arrow-cursor glyph, matches the filled-fill idiom already used by `dot`/`savings`/`live`) and `toolMove` (outline open-hand glyph, same stroke language as the rest). Both `aria-hidden` via the existing `Icon` wrapper — button text label ("Select"/"Move") carries the accessible name, same as every other toolbar button.
- **Delete button**: disabled (not hidden) when nothing is selected, so its position in the toolbar is stable and it's discoverable via Tab even with an empty selection; `disabled` conveys state to AT natively (no extra `aria-disabled` needed).
- **Delete/Backspace key**: guarded against firing while the studio's own text-entry overlay or the new Name/Summary inputs have focus (checks `document.activeElement.tagName`), so users typing a caption containing the word "delete" or backspacing over a typo never lose a shape by accident.
- **Name/Summary inputs**: real `<label>`-wrapped `<input>`s (implicit label association, no separate `aria-label` needed), same visual weight/padding as the rest of the studio's form controls.
- **Selection highlight**: a dashed outline (`C.sel`, 1.5px) drawn on the canvas itself around the selected shape's bounding box — canvas content isn't in the accessibility tree, but the shape being acted on is also named implicitly by tool context (Select tool + Delete button enabling is the operable signal, not the highlight alone).
- Verified via `?mock=1`: box/draw/text shapes select + highlight + delete correctly; Move drags live; Blur redaction confirmed non-destructive (moved a blur off a scribbled region and the original strokes were intact underneath, unedited); undo restores the pre-move state. No console errors on a fresh tab, both the bug-report attach flow and the admin thread render (name → alt text, caption → visible line) correctly. Cursor changes per active tool (default for Select, grab for Move, crosshair for drawing tools) as a non-color affordance of which mode is active, alongside the existing `aria-pressed` toolbar state.
