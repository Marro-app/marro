...
## 2026-07-19 — Presentation polish: loan-card collapse, stale copy, duplicate surplus (branch `mo/polish-presentation`, stacked on `mo/fix-shell-blockers`)

Ten copy/presentation findings from the same break-testing sweep. Verified via `?mock=1`, both themes.

- **Loans tab: every loan card rendered its full ~15-field editor permanently** (`src/tabs/LoansTab.jsx`) — refactored to an accessible disclosure: at rest, a real `<button aria-expanded aria-controls>` shows one summary row (name · amount borrowed · "$X at graduation," or "not yet counted" for an 'offered' loan not yet in the graduation projection), expanding to the existing form on click/Enter. Focus moves into the form's Name field on expand and back to the toggle on collapse (`requestAnimationFrame` after the state update, same idiom as AidTab's year cards). "+ Add loan" creates the new card pre-expanded with the Name field auto-focused. All existing form logic/handlers are untouched — this was a presentation wrapper, not a rewrite (confirmed: `removePart` redistribution tests still pass unmodified). Verified: expand/collapse by mouse and keyboard on 2 mock loans, "Add loan" opens a fresh focused form, delete (✕) still works without expanding.
- **Killed the stale "loan tracking is coming soon" promise** (`src/tabs/AidTab.jsx`, appears once per year card × 4 years) — the Loans tab has existed for a while; now reads "…add them on the Loans tab to see what you'll really owe."
- **Duplicate "Planned surplus" removed from Budget's Cash-flow card** (`src/tabs/BudgetTab.jsx`) — the same number (`runningBalance`, no prior-year carryover in the common case) appeared both as a Cash-flow row and as its own dedicated card two cards down; removed the Cash-flow row, kept the card (which also carries the fuller "prior years vs. this year" breakdown once there's carryover).
- **Unlabeled category %** — each Budget category row's bare "18%" now reads "18% of plan," a one-word addition (no header row added — smaller diff, same clarity).
- **"$15 auto" → "$15/mo · renews automatically"** (Budget's Fixed-monthly-costs row) — fits without wrapping awkwardly at the row's existing width, so the full phrase was used rather than the tighter "/mo" fallback.
- **Refund-nudge term naming — checked, not changed.** The header "Did your {term} refund land?" banner (`src/App.jsx`) already derives the word from `refundNudge.candidate.term` (`.endsWith("-spring")`/`"-fall"`), and `estimateRefunds()`/`refundNudgeState()` (`src/lib/loans.js`) compute that term dynamically per academic-year date math — there's no hardcoded "spring." The finding was likely observed in July, when "spring" is in fact the correct (most recent past) term; confirmed the code path handles a "-fall" candidate identically. No change made.
- **Runway date now shows the year once it's not this calendar year** — added `fmtDayMaybeYear()` (`src/lib/format.js`), used only for the two forward-looking projected dates in the Runway tile (`runOutDate`, `nextRefund.date` in `src/App.jsx`); entry-list dates (always recent/past) keep the existing plain `fmtDay`. 3 new Vitest cases. Verified: mock's run-out date (~Apr 2027, beyond the current 2026 calendar year) now reads "until ~Apr 23, 2027" instead of the ambiguous "until ~Apr 23."
- **HPSL picker label flipped to plain language** (`src/tabs/LoansTab.jsx`) — "School health-professions loan (HPSL / Primary Care / LDS — often 5%)" → "A loan from your school — often 5% (your paperwork may say HPSL, Primary Care, or LDS)."
- **"Money usually arrives in two parts" now adapts to row count** — 1 disbursement row shows "When the money arrives:"; the two-parts sentence only shows at ≥2 rows. Verified both states by adding/removing a disbursement part on a mock loan.
- **Notes card now labels what it's for** — title "Notes" → "Notes for this year" (the field is year-scoped, not month-scoped, which the Budget tab's month-picker context could otherwise make ambiguous), placeholder expanded, and an `aria-label` was added (the textarea previously had none — a real WCAG 2.1 AA gap fixed in passing).
- Tests: 166/166 passing (163 from PR1 + 3 new for `fmtDayMaybeYear`) · build clean · lint 0 errors (109 pre-existing warnings, none new). Both themes verified for every changed surface; one console warning observed (`ReactDOMClient.createRoot()` called twice) traced to the harness's forced same-page reload, not an app bug — same class of mock-harness-only artifact documented in the 2026-07-17 entry above.
