...
- **Admin-facing "Usage" dashboard** (`src/tabs/AdminTab.jsx` → `UsageSection`, added on top of the collection above): shows founders aggregate counts only — most/least-clicked elements, a daily activity trend, and a 30-day active-clicker count — never a per-user click history or any individual's activity feed. The client never gets raw `SELECT` access to `events` or its views; it goes through three `is_admin()`-gated `SECURITY DEFINER` RPCs (`admin_click_by_element`, `admin_daily_event_counts`, `admin_events_last_30_days`, `supabase/analytics.sql` section 3) that re-check admin status server-side on every call and return nothing to anyone else. This is Lane B by construction — the dashboard literally cannot render an individual's click trail, because the RPCs never return `user_id`.
